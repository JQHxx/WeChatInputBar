...
- (void)reloadScrollView {
     ...
     */
    int page = (int)(self.curGroup.facesArray.count / (self.curGroup.faceType == HCDFaceTypeEmoji ? 20 : 8));
    //看还有没有剩余 如果有y 还需要一页来显示
    int more = (int)(self.curGroup.facesArray.count % (self.curGroup.faceType == HCDFaceTypeEmoji ? 20 : 8));
    page += (more > 0 ? 1 : 0);
...
- (void)showFacePageAtIndex:(NSUInteger)index {
    ...
    int count = _curGroup.faceType == HCDFaceTypeEmoji ? 20 : 8;
    if (_curPage == -1) {
